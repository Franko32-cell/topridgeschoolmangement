...
def _admission_trend(months=6):
    ...
    NOTE: Admission model uses application_date, not created_at.
    """
    try:
        cutoff = timezone.now() - timezone.timedelta(days=months * 30)
        rows = (
            Admission.objects
            .filter(application_date__gte=cutoff)
...
def _recent_admissions(limit=5):
    try:
        qs = Admission.objects.order_by("-application_date")[:limit]
        return [
            {
                "id":     a.id,
...
def _fee_summary():
    ...
    try:
        ...
        total_expected = Fee.objects.aggregate(total=Sum("amount"))["total"] or 0
...
class DashboardView(APIView):
    ...
    def get(self, request):
        ...
        data = {
            ...
            "results_entered": _safe_count(Result.objects.filter(year=now.year)),
        ...
        # ── Teacher ───────────────────────────────────────────────────────────
            ...
            try:
                ...
                data.update({
                    ...
                    "my_students":      _safe_count(Student.objects.filter(school_class=sc)) if sc else 0,
                    "attendance_today": _attendance_today(),
                })
        ...
        # ── Student ───────────────────────────────────────────────────────────
            ...
            try:
                ...
                data.update({
                    ...
                    "my_results_count":     _safe_count(Result.objects.filter(student=student, year=now.year)),
